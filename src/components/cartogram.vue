...
export default {
  ...
  methods:{	
      ...
      let coordinateLine = this.h/3;
	    ...
	    let grad  = cartogram.createLinearGradient(0,0, this.width,0),intervalW = this.intervalW/2;
      ...
      this.lineAnimate(cartogram)
		...
		speedLine(cartogram,motionW,motionH,h){
			requestAnimationFrame( () => {  
	      if(this.lineProcess==0){
					cartogram.moveTo(0,h);
	      }else if(this.lineProcess == this.lineStyleNum){
	      	let lineProcess = this.lineProcess
					cartogram.moveTo(lineProcess,h);
          cartogram.lineTo(lineProcess,h-this.OldMotionH);
	      }
	      if(motionW > this.lineProcess){
	      	cartogram.fillStyle = '#f1f5fc';
	      	this.lineProcess =Math.min(this.lineProcess + 30,motionW)
	      	let lineProcess = this.lineProcess
	      	let cz
					if(this.lineNum==0){
					  cz = motionH/motionW*(lineProcess-this.OldMotionW)
					}else{
					  cz =(motionH-this.OldMotionH)/(motionW-this.OldMotionW)*(lineProcess-this.OldMotionW)
					}
					cartogram.lineTo(lineProcess,h-this.OldMotionH-cz);
					cartogram.lineTo(lineProcess,h);
		      this.OldMotionH = this.OldMotionH + cz
		      this.OldMotionW = lineProcess
	      	this.lineStyleNum = this.lineProcess
			    this.speedLine(cartogram,motionW,motionH,h)
	        cartogram.fill(); 
	      }else{  
	      	this.lineNum++;
		...
		lineAnimate(cartogram,isDrawDashedLine){	    
			let h = this.h;
	    let w = this.w;
	    let val = this.val;
	    ...
	    let intervalW = this.intervalW
		    if(this.lineNum ==0){
		      this.speedLine(cartogram,intervalW,val[this.lineNum]*lineH,h)
		    }else{
					this.speedLine(cartogram,this.lineNum * lineW+intervalW,val[this.lineNum]*lineH,h)
		    }
